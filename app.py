...
if tab == "Tableau de bord analytique":
    ...
    total_retraits = df_filtered["total_montant"].sum()
    total_operations = df_filtered["total_nombre"].sum()
    volume_moyen_semaine = df_filtered.groupby("week")["total_montant"].mean().mean() if not df_filtered.empty else 0
    ecart_type_retraits = df_filtered["total_montant"].std() if not df_filtered.empty else 0
    part_weekend = (df_filtered[df_filtered["week_day"]>=5]["total_montant"].sum() / df_filtered["total_montant"].sum() *100) if not df_filtered.empty else 0

    col1, col2, col3, col4, col5 = st.columns(5)
    col1.metric("Nombre de GAB", f"{nb_gab}")
    col2.metric("Montant total retraits", f"{total_retraits/1000:,.0f} KDH")
    col3.metric("Nombre total opérations", f"{total_operations/1000:,.0f} KDH")
    col4.metric("Volume moyen hebdo", f"{volume_moyen_semaine/1000:,.0f} KDH")
